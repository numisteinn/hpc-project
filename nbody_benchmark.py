...
def run_benchmark(implementation, name, N, t_end=1.0, dt=0.01, softening=0.1, G=1.0, 
                  pos=None, vel=None, dask_pos=None, dask_vel=None, torch_pos=None, torch_vel=None):
...
def benchmark_all_implementations(N_values, runs_per_N=3):
    ...
    for N in N_values:
        ...
        for run in range(runs_per_N):
            ...
            try:
                results.append(run_benchmark(
                    original_main, "original", N, pos=pos_np.copy(), vel=vel_np.copy()))
            except Exception as e:
                print(f"Error in original implementation: {e}")
            
            # try:
            #     results.append(run_benchmark(
            #         cython_main, "cython", N, pos=pos_np.copy(), vel=vel_np.copy()))
            # except Exception as e:
            #     print(f"Error in Cython implementation: {e}")
            
            try:
                results.append(run_benchmark(
                    torch_main, "pytorch", N, torch_pos=torch_pos.clone(), torch_vel=torch_vel.clone()))
            except Exception as e:
                print(f"Error in PyTorch implementation: {e}")
            
            try:
                results.append(run_benchmark(
                    dask_main, "dask", N, dask_pos=dask_pos.copy(), dask_vel=dask_vel.copy()))
            except Exception as e:
...
def plot_results(df, output_dir="benchmark_results"):
    ...
    plt.figure(figsize=(12, 8))
    pivot_df = df.pivot_table(index=["N", "implementation"], values="execution_time").reset_index()
    
    # Calculate speedup for each N
    speedups = []
    for n in pivot_df["N"].unique():
        n_df = pivot_df[pivot_df["N"] == n]
        original_time = n_df[n_df["implementation"] == "original"]["execution_time"].values[0]
...
def main():
    ...
    # Print summary
    print("\nBenchmark Summary:")
    summary = results.groupby(["implementation", "N"])["execution_time"].mean().reset_index()
    print(summary)
